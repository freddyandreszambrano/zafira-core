...
from app.security.models import Group, GroupModule, GroupPermission, Module, ModuleType
...
MODULES = [
    {
        ...
        'permits_app': 'security', 'permits_model': 'module',
        'order': 1,
    },
    {
        'type': 'Seguridad',
        ...
        'permits_app': 'security', 'permits_model': 'moduletype',
        'order': 2,
    },
    {
        'type': 'Seguridad',
        ...
        'permits_app': 'security', 'permits_model': 'group',
        'order': 3,
    },
    {
        'type': 'Usuarios',
        ...
        'permits_app': 'users', 'permits_model': 'user',
        'order': 1,
    },
    {
        'type': 'Cuenta',
        ...
        for cfg in MODULES:
            ...
            if cfg.get('permits_app') and cfg.get('permits_model'):
                ct = ContentType.objects.filter(
                    app_label=cfg['permits_app'], model=cfg['permits_model'],
                ).first()
                if ct:
        ...
        for module in modules:
            ...
            for perm in module.permits.all():
                GroupPermission.objects.get_or_create(
                    group=group, module=module, permission=perm,
                )
